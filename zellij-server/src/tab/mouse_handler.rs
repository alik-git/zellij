...
impl MouseHandler {
    ...
    fn determine_mouse_action(event: &MouseEvent, ctx: &MouseEventContext) -> Result<MouseAction> {
        ...
        if event.wheel_up || event.wheel_down {
            if let Some(pane_id) = ctx.pane_id_at_position {
                let lines = if event.ctrl {
                    CTRL_MOUSE_SCROLL_LINES
                } else {
                    ctx.mouse_scroll_lines
                };
                if event.wheel_up {
                    return Ok(MouseAction::ScrollUp { pane_id, lines });
                }
                if event.wheel_down {
                    return Ok(MouseAction::ScrollDown { pane_id, lines });
                }
            }
            return Ok(MouseAction::NoAction);
        }
